...
for epoch in range(num_epochs):
    for batch_idx, (real, _) in enumerate(train_loader):
        ...
        if batch_idx % 100 == 0:
            ...
            real_accuracy = torch.mean((outputs_real > 0.5).float())
            fake_accuracy = torch.mean((outputs_fake <= 0.5).float())

            print(f"Accuracy real: {real_accuracy:.4f}, fake: {fake_accuracy:.4f}")
